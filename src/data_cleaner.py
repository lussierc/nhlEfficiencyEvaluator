                ...
                if all_teams[home_team_name]:
                    ...
                    for game_player_key in home_team.keys():
                        if game_player_key not in all_teams[home_team_name].keys():
                            ...
                            all_teams[home_team_name][game_player_key] = home_team[game_player_key]
                            ...
                            if all_teams[home_team_name][game_player_key]['stats'] == '':
                                ...
                                season_dict = all_teams[home_team_name][game_player_key]['stats']
                ...
                if all_teams[away_team_name]:
                    ...
                    for game_player_key in away_team.keys():
                        if game_player_key not in all_teams[away_team_name].keys():
                            # if a player does not exist on team, add them to it:
                            all_teams[away_team_name][game_player_key] = away_team[game_player_key]
                            ...
                            if all_teams[away_team_name][game_player_key]['stats'] == '':
                                ...
                                # converge stats from new/current game with season stats
                                season_dict = all_teams[away_team_name][game_player_key]['stats']
...
def get_game_data(data):
    if 'liveData' in data:
        ...
        for key in home_team_players.keys():
            ...
            cur_player = {'name': player_name, 'stats': '', 'position': player_postition}
            ...
            if player_stats:
                ...
                if 'skaterStats' in player_stats.keys():
                    ...
                    cur_player['stats']['timeOnIce'] = int(cur_player['stats']['timeOnIce'].split(':')[0])
                    cur_player['stats']['evenTimeOnIce'] = int(cur_player['stats']['evenTimeOnIce'].split(':')[0])
                    cur_player['stats']['powerPlayTimeOnIce'] = int(cur_player['stats']['powerPlayTimeOnIce'].split(':')[0])
                    cur_player['stats']['shortHandedTimeOnIce'] = int(cur_player['stats']['shortHandedTimeOnIce'].split(':')[0])
